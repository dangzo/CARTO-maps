...
it('should update layer fill color', () => {
  const state = layerControlsReducer(
    initialState,
    updateLayerFillColor({ layerIndex: 0, value: '#ff0000' })
  );
...
it('should update layer outline size', () => {
  const state = layerControlsReducer(
    initialState,
    updateLayerOutlineSize({ layerIndex: 1, value: 3 })
  );
...
it('should update layer outline color', () => {
  const state = layerControlsReducer(
    initialState,
    updateLayerOutlineColor({ layerIndex: 0, value: '#00ff00' })
  );
...
it('should update layer radius', () => {
  const state = layerControlsReducer(
    initialState,
    updateLayerRadius({ layerIndex: 1, value: 5 })
  );
...
it('should update layer fill by', () => {
  const state = layerControlsReducer(
    initialState,
    updateLayerFillBy({ layerIndex: 0, value: 'population' })
  );
...
it('should handle multiple updates independently', () => {
  let state = layerControlsReducer(initialState, updateLayerFillColor({ layerIndex: 0, value: '#aabbcc' }));
  state = layerControlsReducer(state, updateLayerOutlineSize({ layerIndex: 1, value: 2 }));
