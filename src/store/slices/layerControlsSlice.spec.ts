...
describe('layerControlsSlice', () => {
  ...
  it('should update layer palette', () => {
    ...
    expect(state.layers[1].palette).toBe('Burg');
    expect(state.layers[0].palette).toBe(INITIAL_LAYER_1_STYLE.palette);
  });

  it('should handle multiple updates independently', () => {
    let state = layerControlsReducer(initialState, updateLayerFillColor({
      layerIndex: 0,
      value: '#aabbcc',
    }));
    state = layerControlsReducer(state, updateLayerOutlineSize({
      layerIndex: 1,
      value: 2,
    }));
